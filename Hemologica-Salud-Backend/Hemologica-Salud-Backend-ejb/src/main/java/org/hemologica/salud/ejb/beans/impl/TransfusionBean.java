...
public class TransfusionBean implements TransfusionBeanLocal, Serializable {
	...
	@Override
	public List<DataTransfusion> getTransfusionsUserId(Long userId) throws XMLDataBaseException, SAXException,
		...
		for (PersonsRecord personsRecord : cdasIds) {
			...
			String cda = XMLDataBaseFactory.getIXMLDataBaseTransfusions().getElementCDAId(
	...
	private DataTransfusion getDataTransfusion(Document document)
		 ...
		 * Id bolsa
		 */
		data.setProductCode(XMLUtils.executeXPathString(document,
				"//ClinicalDocument//component//structuredBody//component//section//entry//procedure//specimen[descendant-or-self::node()//@code=\"122564002\"]//id//@extension"));
		...
		List<Document> components = XMLUtils.executeXPathStringList(document,
				"/ClinicalDocument/component/structuredBody/component/section/entry/procedure");
		if (components != null && components.size() == 1) {

			Document d = components.get(0);
			// String procedure =
			// XMLUtils.executeXPathString(document,"/ClinicalDocument/component/structuredBody/component/section/entry/procedure");

			// String componentCode = XMLUtils.executeXPathString(d,
			...
			data.setDataProduct(FactoryBeans.getCodeBeans().getProductBySnomedCode(XMLUtils.executeXPathString(d,
					"//procedure//entryRelationship[descendant-or-self::node()/@typeCode = \"COMP\"]//observation//@code")));
			...
			if (eventsDoc != null) {
				...
				for (Document docEvent : eventsDoc) {
					...
					String severity = XMLUtils.executeXPathString(docEvent,
		...
		List<Document> componentAnalysis = XMLUtils.executeXPathStringList(document,
		...
		for (Document d : componentAnalysis) {
			...
			dataLaboratoryResult.setAnalysis(FactoryBeans.getCodeBeans().getTransfusionAnalysisBySnomedCode(
					XMLUtils.executeXPathString(d, "/component/observation/entryRelationship/observation/code/@code")));
			dataLaboratoryResult.setResult(FactoryBeans.getCodeBeans().getResultBySnomedCode(XMLUtils
					.executeXPathString(d, "/component/observation/entryRelationship/observation/value/@code")));
	...
	public DataResponse addTransfusion(DataTransfusion dataTransfusion) {
		...
		ClinicalDocumentType clinicalDocumentType = CdaUtils.getCDAStructure(dataTransfusion.getPerson(),
				dataTransfusion.getTime(), dataTransfusion.getBank(),
				dataTransfusion.getResponsibleTransfusionPerson());
	...
	@Override
	public DataTransfusion getDataTransfusionSpecimenId(String code, String institutionCode)
	...
	@Override
	public List<DataTransfusion> getTransfusionsFilters(List<DataSearchFilter> filters)
			throws SAXException, IOException, ParserConfigurationException, XPathExpressionException, DOMException {
	...
	private DataPerson getDataPersonFromDocument(Document document) throws XPathExpressionException {
		...
		// String root = XMLUtils.executeXPathString(document,
		// "//ClinicalDocument//author//assignedAuthor//representedOrganization//id//@root");
		// String extension = XMLUtils.executeXPathString(document,
		// "//ClinicalDocument//author//assignedAuthor//representedOrganization//id//@root");
